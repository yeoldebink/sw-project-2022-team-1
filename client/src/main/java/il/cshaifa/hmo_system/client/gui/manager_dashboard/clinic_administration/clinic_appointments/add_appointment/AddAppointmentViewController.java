...
public class AddAppointmentViewController extends ViewController {
    ...
    void requestCreateAppointments(ActionEvent actionEvent) {
        ...
        try {
            Time time_value = new Time(new SimpleDateFormat("HH:mm").parse(start_time.getText()).getTime());
            LocalDateTime start_datetime = LocalDateTime.of(start_date.getValue(), time_value.toLocalTime());
            Integer count_appointments = Integer.parseInt((num_appts.getText()));

            EventBus.getDefault().post(new AddAppointmentEvent(this.staff_member, this.clinic, start_datetime, count_appointments, Phase.SEND));
