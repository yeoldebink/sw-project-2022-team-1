 ...
 * </code> class. This means that when a message is received, all observers are notified.
 *
...
public class ObservableServer extends Observable {
  ...
  /**
   * The string sent to the observers when an exception occurred with a client. The error message of
   * that exception will be appended to this string.
   */
  public static final String CLIENT_EXCEPTION = "#OS:Client exception.";

  /**
   * The string sent to the observers when a listening exception occurred. The error message of that
   * exception will be appended to this string.
   */
  ...
  /**
   * Returns an array of containing the existing client connections. This can be used by concrete
  ...
  /**
   * Sets the port number for the next connection. Only has effect if the server is not currently
   * listening.
   *
  ...
  /**
   * Sets the timeout time when accepting connection. The default is half a second. The server must
   * be stopped and restarted for the timeout change be in effect.
   *
   ...
   * 20. The server must be closed and restart for the backlog change be in effect.
   *
  ...
  /**
   * Hook method called each time a new client connection is accepted. The method may be overridden
   * by subclasses.
   *
  ...
  /**
   * Hook method called each time a client disconnects. The method may be overridden by subclasses.
   *
  ...
  /**
   * Hook method called each time an exception is raised in a client thread. This implementation
   * simply closes the client connection, ignoring any exception. The method may be overridden by
   * subclasses.
   *
  ...
  /**
   * This method is called when the server stops accepting connections because an exception has been
   * raised. This implementation simply calls <code>stopListening</code>. This method may be
  ...
  /**
   * This method is called when the server stops accepting connections for any reason. This method
  ...
  /**
   * This method is called when the server is closed. This method may be overriden by subclasses.
   */
  ...
  /**
   * This method is called when the server starts listening for connections. The method may be
  ...
  /**
   * This method is used to handle messages coming from the client. Observers are notfied by
   * receiveing the transmitted message. Note that, in this implementation, the information
   * concerning the client that sent the message is lost. It can be overriden, but is still expected
   * to call notifyObservers().
   *
