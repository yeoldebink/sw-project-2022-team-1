...
package il.cshaifa.hmo_system.server.ocsf;
...
/**
 * The <code> AbstractServer </code> class maintains a thread that waits for connection attempts
 * from clients. When a connection attempt occurs it creates a new <code> ConnectionToClient </code>
 * instance which runs as a thread. When a client is thus connected to the server, the two programs
 * can then exchange <code> Object </code> instances.
 *
 * <p>Method <code> handleMessageFromClient </code> must be defined by a concrete subclass. Several
 * other hook methods may also be overriden.
 *
 * <p>Several public service methods are provided to applications that use this framework, and
 * several hook methods are also available
 *
 * <p>The modifications made to this class in version 2.2 are:
 *
 * <ul>
 *   <li>The synchronization of the <code>close()</code> method is now limited to the client threads
 *       closing sequence. The call to <code>serverClosed()</code> is outside the synchronized block
 *       and is preceeded by a join that garantees that <code>serverStopped()</code> is always
 *       called before.
 *   <li>Method <code>isClosed()</code> has been added.
 *   <li>When a client is accepted, the corresponding connection thread will be created only if the
 ...
 *
 * <ul>
 *   <li>An instance variable refering to the current connection factory. Refer to null value by
 *       default, in this case regular <code>ConnectionToClient</code> instances are created as in
 *       the previous versions.
 *   <li>Method <code>setConnectionFactory()</code> has been added.
 *   <li>In the run method, a call to the connection factory is made if such a factory is available.
 *   <li>Method <code>handleMessageFromClient</code> is not always called depending on the value
 *       returned by the <code>handleMessageFromClient</code> of the <code>ConnectionToClient</code>
 *       class.
 *   <li>The <code>clientException</code> method is still the one called when an exception is thrown
 *       when handling the connection with one client. However <code>ClassNotFoundException</code>
 *       and <code>RuntimeException</code> instances can now be received.
 *   <li>The call to <code>serverStopped()</code> has been moved in the <code>run</code> method.
...
public abstract class AbstractServer implements Runnable {
  ...
  /**
   * The factory used to create new connections to clients. Is null by default, meaning that regular
   * <code>ConnectionToClient</code> instances will be created. Added in version 2.3
   */
  ...
  /**
   * Sends a message to every client connected to the server. This is merely a utility; a subclass
   * may want to do some checks before actually sending messages to all clients. This method can be
   * overriden, but if so it should still perform the general function of sending to all clients,
   * perhaps after some kind of filtering is done. Any exception thrown while sending the message to
   * a particular client is ignored.
   *
  ...
  /**
   * Sets the port number for the next connection. The server must be closed and restarted for the
   * port change to be in effect.
   *
  ...
  /**
   * Sets the timeout time when accepting connections. The default is half a second. This means that
   * stopping the server may take up to timeout duration to actually stop. The server must be
   * stopped and restarted for the timeout change to be effective.
   *
  ...
  /**
   * Sets the maximum number of waiting connections accepted by the operating system. The default is
   * 20. The server must be closed and restarted for the backlog change to be in effect.
   *
  ...
  /**
   * Sets the connection factory. Once set, this one will be used in the creation of new <code>
   * ConnectionToClient</code> instances. The call to this method is optional; if not called Then
   * regular <code>ConnectionToClient</code> instances are created. Added in version 2.3
   *
  ...
  /**
   * Receives a command sent from the client to the server. Called by the run method of <code>
   * ConnectionToClient</code> instances that are watching for messages coming from the server This
   * method is synchronized to ensure that whatever effects it has do not conflict with work being
   * done by other threads. The method simply calls the <code>handleMessageFromClient</code> slot
   * method.
   *
   * @param msg the message sent.
   * @param client the connection connected to the client that sent the message.
