...
package il.cshaifa.hmo_system.client_base.ocsf;
 ...
 *
 * <p>Method <code> handleMessageFromServer </code> must be defined by a concrete subclass. Several
 * other hook methods may also be overriden.
 *
 * <p>Several public service methods are provided to application that use this framework.
 *
 ...
 *       the server. However, any overriden version of this method should include a call to the
 *       original one.
 *   <li>A test is made before calling the <code>handleMessageFromServer</code> method such that
 ...
 *
 * <ul>
 *   <li>The <code>run()</code> method now calls the <code>connectionException</code> callback when
 *       an object of unknown class is received from the input stream or when the message handler
 *       throw a <code>RuntimeException</code>.
 *   <li>The <code>connectionClosed</code> callback might be called after <code>connectionException
 *       </code> if the exception causes the end of te thread.
 *   <li>The <code>clientReader</code> reference is set to <code>null</code> earlier in <code>run()
 *       </code> method.
 *   <li>The call to <code>connectionClosed</code> has been moved from <code>closeConnection</code>
 *       to <code>run</code> method to garantee that connection is really closed when this callback
 *       is called.
 * </ul>
 *
...
public abstract class AbstractClient implements Runnable {
  ...
  /**
   * Waits for messages from the server. When each arrives, a call is made to <code>
   * handleMessageFromServer()</code>. Not to be explicitly called.
   */
  ...
  /**
   * Handles a message sent from the server to this client. This MUST be implemented by subclasses,
